/*
*3D_tube_poiseuille_dop_outflow.h
*
...
// output folders
std::string OUTPUT_FOLDER = "output/3dtpdop/Re3500/";
std::string OUTPUT_FOLDER_VTU = OUTPUT_FOLDER + "vtu/";
std::string OUTPUT_NAME = "tube_dataofpatients_outflow";
std::string FILENAME_FLOWRATE = "tube_mean_velocity";
...
double const LENGTH_PRECURSOR = 0.312;

// mesh parameters
unsigned int const N_CELLS_AXIAL = 2;
unsigned int const N_CELLS_AXIAL_PRECURSOR = 4*N_CELLS_AXIAL;

double const Z1_PRECURSOR = 0;
double const Z2_PRECURSOR = LENGTH_PRECURSOR;
...
double const MAX_VELOCITY = 15.09;

// start and end time
double const START_TIME = 0; // Let the flow develop
double const END_TIME = 30.0; //194.0;

// output
bool const WRITE_OUTPUT = true;
double const OUTPUT_START_TIME = START_TIME;
double const OUTPUT_INTERVAL_TIME = 1;  //10.0*T_0;

// sampling

// sampling interval should last over (100-200) * T_0 according to preliminary results.
//double const SAMPLE_START_TIME = 0; // let the flow develop
...
double const C_RS_KINEMATIC = DENSITY * 0.7838e-6; // DENSITY * 100.0e-6/98.0665;   // total respiratory compliance C_rs = 100 ml/cm H20
    ...
    std::shared_ptr<BoundaryDescriptorP<dim> > boundary_descriptor_pressure)
{
  ...
  boundary_descriptor_pressure->dirichlet_bc.insert(pair(1,new PressureInflowBC<dim>(1)));
